...
export async function POST(_req: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    // Debounce: check for active t2_semantic job on this source
    ...
    const job = await enqueue('t2_semantic', object.source_id, scope, 'on_demand', orgId);

    return NextResponse.json({ data: { jobId: job.id }, queued: true }, { status: 202 });
  } catch (err) {
    console.error('[context/objects/:id/enrich POST]', err);
