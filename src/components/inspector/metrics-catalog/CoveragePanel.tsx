...
/**
 * Coverage — governed / candidate / draft by theme. Bars ⇄ Treemap. A theme
 * click sets the Theme facet; "Gaps only" filters to under-governed themes.
 *
 * On the live estate this reads as a near-ungoverned wall of candidate — which
 * is the governance backlog the catalog exists to surface, not a bug. The gap
 * flags point straight at the work.
 */
export function CoveragePanel() {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${BORDER}`, padding: '12px 20px 14px', flexShrink: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ ...SERIF, fontSize: 15, fontWeight: 600, color: INK }}>
          Coverage
          <span style={{ ...MONO, fontSize: 10, color: MUTED, marginLeft: 10, fontWeight: 400 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button
            ...
            style={{
              display: 'flex', alignItems: 'center', gap: 5, ...MONO, fontSize: 10,
      ...
      {mode === 'bars' ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {buckets.map((b) => (
            <CoverageBar key={b.theme} bucket={b} active={themeFacet.includes(b.theme)} onClick={() => setThemeFacet(b.theme)} />
          ))}
        </div>
      ) : (
...
function ModeBtn({ active, onClick, icon }: { active: boolean; onClick: () => void; icon: React.ReactNode }) {
  return (
    <button
      ...
      aria-pressed={active}
      style={{
        display: 'flex', alignItems: 'center', padding: '3px 7px', borderRadius: 3, border: 'none', cursor: 'pointer',
        background: active ? GOLD : 'transparent', color: active ? '#1A1206' : MUTED,
      }}
    >
      {icon}
    </button>
...
function CoverageBar({ bucket, active, onClick }: { bucket: CoverageBucket; active: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      style={{
        display: 'grid', gridTemplateColumns: '180px 1fr 92px', alignItems: 'center', gap: 12,
        width: '100%', background: active ? 'rgba(253,181,21,0.06)' : 'transparent',
        border: 'none', borderRadius: 4, padding: '3px 6px', cursor: 'pointer', textAlign: 'left',
      }}
    >
      <span style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
        {bucket.gap && <AlertTriangle size={11} color={GOLD} style={{ flexShrink: 0 }} />}
        <span style={{ ...MONO, fontSize: 11, color: active ? GOLD : INK_DIM, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {bucket.theme}
        </span>
        <span style={{ ...MONO, fontSize: 9, color: MUTED, flexShrink: 0 }}>{bucket.total}</span>
      </span>
      <span style={{ display: 'flex', height: 12, borderRadius: 3, overflow: 'hidden', background: 'rgba(255,255,255,0.04)' }}>
      ...
      {buckets.map((b) => {
        ...
        return (
          <button
            ...
            style={{
              flex: `1 1 ${basis}px`, minHeight: 54, borderRadius: 4, padding: '8px 10px', cursor: 'pointer',
